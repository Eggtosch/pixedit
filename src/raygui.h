...
int GuiGrid(Rectangle bounds, const char *text, float spacing, int subdivs, Vector2 *mouseCell)
{
    ...
    if (subdivs > 0)
    {
        ...
        for (int i = 0; i < linesV; i++)
        {
            ...
            GuiDrawRectangle(lineV, 0, BLANK, ((i%subdivs) == 0)? GuiFade(GetColor(color), RAYGUI_GRID_ALPHA*8) : GuiFade(GetColor(color), RAYGUI_GRID_ALPHA*4));
        }
        ...
        for (int i = 0; i < linesH; i++)
        {
            ...
            GuiDrawRectangle(lineH, 0, BLANK, ((i%subdivs) == 0)? GuiFade(GetColor(color), RAYGUI_GRID_ALPHA*8) : GuiFade(GetColor(color), RAYGUI_GRID_ALPHA*4));
        }
    }
